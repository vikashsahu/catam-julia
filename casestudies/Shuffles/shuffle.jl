...
# ╔═╡ ce37949c-fa81-11eb-2c5f-5356851bed48
md"""
# The Statistics of Card Shuffles
"""
...
displayDeck(deck) = [RGB{Float32}(x/52,0.3,1-x/52) for x in deck]

# ╔═╡ 9d80001b-b967-4cab-b785-978a8c4055cb
displayDeck(deck)

# ╔═╡ 9a596318-209c-4eef-b8a9-50cbb11b2cc0
md"""
## Simulating Card Shuffles
"""
...
function riffle(deck, n = 1)
	if n == 0 return copy(deck) end
	bits = rand(Bool, 52)
	newDeck = zeros(Int, 52)
	(l, r) = (1, sum(bits)+1)
	for i in 1:52
		if bits[i] == 1
			newDeck[i] = deck[l]
...
function overhand(deck, n = 1, p = 0.2)
	if n == 0 return copy(deck) end
	cuts = push!([i for i in 51:-1:1 if rand() < p], 0)
	newDeck = []
	lastcut = 52
	for c in cuts
		append!(newDeck, deck[c+1:lastcut])
		lastcut = c
	end
...
md"""
##### Top Card Distribution of the Riffle Shuffle
"""

# ╔═╡ 2c73ded2-216a-44e0-8c21-c816c289bec3
begin
plot(xlims = (1,52), xlabel = "Position", ylabel = "Frequency")
plot!(topCardDist(riffle, 1, 1000000), label = "1 Iteration")
plot!(topCardDist(riffle, 2, 1000000), label = "2 Iterations")
plot!(topCardDist(riffle, 3, 1000000), label = "3 Iterations")
plot!(topCardDist(riffle, 4, 1000000), label = "4 Iterations")
plot!(topCardDist(riffle, 5, 1000000), label = "5 Iterations")
plot!(topCardDist(riffle, 6, 1000000), label = "6 Iterations")
plot!(topCardDist(riffle, 7, 1000000), label = "7 Iterations")
end

# ╔═╡ 637e1c56-feb2-42e0-9c39-f7ad33616d31
begin
plot(legend = false, xticks = 1:10, ylims = (-0.05,1))
plot!(ylabel = "Kullback–Leibler Divergence", xlabel = "Iterations")
plot!([KL(topCardDist(riffle, n, 100000)) for n in 1:10])
end

# ╔═╡ 96e6723a-0fa7-4cd3-a9b4-fb1fa0dffa40
md"""
As expected, with increased number of iteration the final position of the top card tends to be uniformly distributed. From this data it seems like after 7 iterations of the riffle shuffle there is not much to be gained from doing more iterations.
"""

# ╔═╡ 16077fbd-689f-45bb-b063-1b4aeefe8c59
md"""
##### Top Card Distribution of the Overhand Shuffle
"""

# ╔═╡ eae5a4eb-cc4e-402a-b96c-e036c6838b64
begin
plot(xlims = (1,52), xlabel = "Position", ylabel = "Frequency")
plot!(topCardDist(overhand, 1, 1000000), label = "1 Iteration")
plot!(topCardDist(overhand, 2, 1000000), label = "2 Iterations")
plot!(topCardDist(overhand, 3, 1000000), label = "3 Iterations")
plot!(topCardDist(overhand, 4, 1000000), label = "4 Iterations")
plot!(topCardDist(overhand, 5, 1000000), label = "5 Iterations")
plot!(topCardDist(overhand, 6, 1000000), label = "6 Iterations")
plot!(topCardDist(overhand, 7, 1000000), label = "7 Iterations")
end
...
begin
plot(xticks = 1:2:41, ylims = (-.05,1))
plot!(ylabel = "Kullback–Leibler Divergence", xlabel = "Iterations")
plot!([KL(topCardDist(overhand, n, 10000)) for n in 1:41], label = "Overhand Shuffle")
